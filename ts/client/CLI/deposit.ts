...
const argv = yargs(hideBin(process.argv))
  ...
  .command('deposit', 'Deposit funds into OpenOrders account', (yargs) => {
    yargs
...
async function main() {
  ...
  try {
    ...
    const baseAmountNative = toNative(argv.baseAmount as number, marketAccount.baseDecimals);
    const quoteAmountNative = toNative(argv.quoteAmount as number, marketAccount.quoteDecimals);

    console.log('Preparing token deposit...');
